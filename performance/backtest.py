...
def run_historical_backtest(start_date=None, end_date=None):
    ...
    # Markov failure summary
    from risk import get_markov_stats
    markov_stats = get_markov_stats()
    if markov_stats["total"] > 0:
        fail_rate = markov_stats["failures"] / markov_stats["total"] * 100
        print(f"  Markov failures: {markov_stats['failures']}/{markov_stats['total']} ({fail_rate:.1f}%)", flush=True)
        if fail_rate > 50:
            print(f"  WARNING: High Markov failure rate ({fail_rate:.1f}%)", flush=True)
